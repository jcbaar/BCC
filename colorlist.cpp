...
// Color indexes.
int ColorList::m_nIndex[] = 
{
	CARR_TEXT,
	CARR_BACKGROUND,
	CARR_BACKGROUND_READONLY,
	CARR_SELECTION_MARGIN,
	CARR_NUMBER,
	CARR_DELIMITER,
	CARR_SELECTED_TEXT,
	CARR_SELECTED_BKGND,
	CARR_LINE_NUMBERS,
	CARR_BACKGROUND_LINE_NUMBERS,
	CARR_BACKGROUND_NUMBER,
	CARR_BACKGROUND_DELIMITER,
	CARR_BRACKET_MATCH,
	CARR_HYPERLINK,
	CARR_BACKGROUND_HYPERLINK,
	CARR_COLUMN_OVERFLOW
};
...
ColorList::ColorList()
{
	...
	m_Pattern.CreateBitmap( 8, 8, 1, 1, &aPattern );
}
...
BOOL ColorList::SetCommonList()
{
	...
	m_crDefaults[ 4  ] = GetSysColor( COLOR_WINDOWTEXT );
	m_crDefaults[ 5  ] = GetSysColor( COLOR_WINDOWTEXT );
	m_crDefaults[ 6  ] = GetSysColor( COLOR_HIGHLIGHTTEXT );
	m_crDefaults[ 7  ] = GetSysColor( COLOR_HIGHLIGHT );
	m_crDefaults[ 8  ] = RGB( 0, 128, 128 );
	m_crDefaults[ 9  ] = ::GetSysColor( COLOR_WINDOW );
	m_crDefaults[ 10 ] = CLR_DEFAULT;
	m_crDefaults[ 11 ] = CLR_DEFAULT;
	m_crDefaults[ 12 ] = col; 
	m_crDefaults[ 13 ] = RGB( 0, 0, 255 );
	m_crDefaults[ 14 ] = CLR_DEFAULT;
	m_crDefaults[ 15 ] = RGB( 255, 244, 247 );

	// Reset the current contents.
	ResetContent();
	int i = 0;
	while ( m_dwFlags[ i ] != ( DWORD )0xFFFFFFFF )
	{
		// Store default into the parser.
		if ( m_pParser->dwColorFlags & m_dwFlags[ i ] )
			m_pParser->crColors[ m_nIndex[ i ]] = m_crDefaults[ i ];
...
LRESULT ColorList::OnReflectedDrawItem( UINT nID, LPDRAWITEMSTRUCT pDis, BOOL& bNotifyParent )
{
	...
	else
	{
		...
		if ( pDis->itemData )
		{
			...
			// Default color? If so adjust the label to reflect this.
			if ( m_pParser->dwColorFlags & m_dwFlags[ pDis->itemID ] ) name += _T( '*' );

			// Render the item.
			DrawColorEntry( pDis, m_pParser->dwColorFlags & m_dwFlags[ pDis->itemID ] ? m_crDefaults[ pDis->itemID ] : m_pParser->crColors[ m_nIndex[ pDis->itemID ]], name );
			return TRUE;
		}
	}
	return FALSE;
...
void ColorList::DrawColorEntry( LPDRAWITEMSTRUCT pDis, COLORREF crColor, LPCTSTR pszName )
{
	...
	// Is it a "CLR_DEFAULT" color?
	if ( crColor == CLR_DEFAULT )
	{
		// Yes. Draw it with a pattern brush to mark it as
		// being transparent.
		brush.CreatePatternBrush( m_Pattern );
	}
	else
	{
		// Create the color brush.
