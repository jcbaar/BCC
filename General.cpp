...
void GeneralPage::RefreshData( LPPARSER pParser )
{
	...
	SetDlgItemInt( IDC_LINEMARGIN, pParser->nMarginChars, FALSE );
	SetDlgItemInt( IDC_SCROLL_LINES, pParser->nScrollLines, FALSE );

	// And the other controls.
	m_Caret.SetCurSel( pParser->nCaretType );
	m_UDHops.SetPos32( pParser->nUndoSize );
	m_UDHist.SetPos32( pParser->nMaxTrack );
	m_UDMarkerOfs.SetPos32( pParser->nMarkerOffset );
	m_UDLineMargin.SetPos32( pParser->nMarginChars );
	m_UDScrollLines.SetPos32( pParser->nScrollLines );
...
BOOL GeneralPage::SavePage( ClsStdioFile *pFile, LPPARSER pParser )
{
	try
	{
		...
		pFile->PrintF( _T( "SaveTabsAsSpaces=%lc\n" ),	    pParser->bSaveTabsAsSpaces	  ? _T( 'Y' ) : _T( 'N' ));
		...
		pFile->PrintF( _T( "ColorColumnOverflow=%lc\n" ),   pParser->bColorColumnMarkerOverflow	  ? _T( 'Y' ) : _T( 'N' ));
		pFile->PrintF( _T( "ColumnMarkerOffset=%ld\n" ),    pParser->nMarkerOffset );
		pFile->PrintF( _T( "LineMarginChars=%ld\n" ),	    pParser->nMarginChars );

		// ScrollLines.
		pFile->PrintF( _T( "ScrollLines=%ld\n" ),	    pParser->nScrollLines );
...
LRESULT GeneralPage::OnCommand( UINT nNotifyCode, UINT nCtrlID, HWND hWndCtrl )
{
	...
	switch ( nCtrlID )
	{
		...
		case	IDC_LINEMARGIN:
		{
			...
			{
				// Get value.
				int nValue = GetDlgItemInt( IDC_LINEMARGIN, NULL, FALSE );
				...
				if ( nValue < 3 || nValue > 10 )
				{
					// No. Give a message and reset the value.
					ClsRect rc;
					m_LineMagin.GetWindowRect( rc );
					ScreenToClient( rc );
					ClsBalloonHelp::LaunchBalloon( ClsGetApp()->GetAppTitle(), MAKEINTRESOURCE( IDS_ERROR_LINEMARGIN ), rc.CenterPoint(), IDI_WARNING, ClsBalloonHelp::unSHOW_INNER_SHADOW | ClsBalloonHelp::unCLOSE_ON_MOUSE_MOVE, this );
					SetDlgItemInt( IDC_LINEMARGIN, m_pParser->nMarginChars, FALSE );
					m_LineMagin.SetSel( 0, m_LineMagin.GetWindowTextLength());
					m_LineMagin.SetFocus();
					return 0;
				}

				// Set the value.
				m_pParser->nMarginChars = nValue;
			}
			// Value changed?
			else if ( nNotifyCode == EN_CHANGE && ( ::GetFocus() == GetDlgItemHandle( nCtrlID )))			
				pSettings->Changed( m_pParser );
		...
		case	IDC_SCROLL_LINES:
		{
			// Loosing focus?
			if ( nNotifyCode == EN_KILLFOCUS )
			{
				// Get value.
				int nValue = GetDlgItemInt( IDC_SCROLL_LINES, NULL, FALSE );
				...
				if ( nValue < 1 || nValue > 10 )
				{
					// No. Give a message and reset the value.
					ClsRect rc;
					m_ScrollLines.GetWindowRect( rc );
					ScreenToClient( rc );
					ClsBalloonHelp::LaunchBalloon( ClsGetApp()->GetAppTitle(), MAKEINTRESOURCE( IDS_ERROR_SCROLL_LINES ), rc.CenterPoint(), IDI_WARNING, ClsBalloonHelp::unSHOW_INNER_SHADOW | ClsBalloonHelp::unCLOSE_ON_MOUSE_MOVE, this );
					SetDlgItemInt( IDC_SCROLL_LINES, m_pParser->nScrollLines, FALSE );
					m_ScrollLines.SetSel( 0, m_ScrollLines.GetWindowTextLength());
					m_ScrollLines.SetFocus();
					return 0;
				}

				// Set the value.
				m_pParser->nScrollLines = nValue;
			}
			// Value changed?
			else if ( nNotifyCode == EN_CHANGE && ( ::GetFocus() == GetDlgItemHandle( nCtrlID )))			
				pSettings->Changed( m_pParser );
...
LRESULT GeneralPage::OnInitDialog( PROPSHEETPAGE * pPropSheetPage )
{
	...
	m_LineMagin.Attach( GetDlgItemHandle( IDC_LINEMARGIN ));
	m_ScrollLines.Attach( GetDlgItemHandle( IDC_SCROLL_LINES ));
	...
	m_UDLineMargin.Attach( GetDlgItemHandle( IDC_UD_LINEMARGIN ));
	m_UDScrollLines.Attach( GetDlgItemHandle( IDC_UD_SCROLL_LINES ));
	...
	m_UDLineMargin.SetRange32( 3, 10 );
	m_UDScrollLines.SetRange32( 1, 10 );
