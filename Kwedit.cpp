...
LRESULT KWEdit::OnInitDialog( LPPROPSHEETPAGE pPsp )
{
	...
	m_crOrigFg = m_Edit.GetColor( BCC_TEXT );
	m_crOrigBk = m_Edit.GetColor( BCC_BACKGROUND );
...
LRESULT KWEdit::OnCommand( UINT nNotifyCode, UINT nCtrlID, HWND hWndCtrl )
{
	...
	switch ( nCtrlID )
	{
		case	IDC_CANCEL:
			// Cancel.
			m_Edit.SetColor( BCC_TEXT, m_crOrigFg );
			m_Edit.SetColor( BCC_BACKGROUND, m_crOrigBk );
			EndDialog( FALSE );
			return 0;
