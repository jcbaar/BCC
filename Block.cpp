...
LRESULT BlockEdit::OnInitDialog( LPPROPSHEETPAGE pPsp )
{
	...
	m_Esc.Attach( GetDlgItemHandle( IDC_SUPESC ));
	m_Esc.SetCheck( m_Block.bSuppressEscape ? BST_CHECKED : BST_UNCHECKED );
...
LRESULT BlockEdit::OnCommand( UINT nNotifyCode, UINT nCtrlID, HWND hWndCtrl )
{
	...
	switch ( nCtrlID )
	{
		...
		case	IDC_SUPESC:
			// Setup suppress escape character check boolean.
			m_Block.bSuppressEscape = ( BOOL )( m_Esc.GetCheck() == BST_CHECKED );
			break;
